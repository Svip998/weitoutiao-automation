...
description: |
  ...
  版本：v1.1（2026-09-24）— 新增间隔检验（强制），防止高频发布触发平台限流。含 Pexels 搜图、JPEG 校验、去AI味写作、CDP 全流程选择器、发布后验证。
version: 1.1.0
---
...
**每次执行任务前必须先检验，距上一条微头条发布时间不足1小时则跳过本次任务，不发布。**
...
1. **导航到管理页**：
   ...
   Body: https://mp.toutiao.com/profile_v4/weitoutiao
   ```
...
2. **提取最近一条发布时间**：
   ...
   (function(){
     ...
     for(var i=0; i<lines.length; i++){
       var m = lines[i].match(/(\d{2}-\d{2}\s+\d{2}:\d{2})/);
       if(m){
         return JSON.stringify({lastPostTime: m[1], found: true});
       }
     }
...
3. **判定逻辑**：
   - 当前时间 - 最近发布时间 ≥ 1小时 → 继续执行第一步
   - 当前时间 - 最近发布时间 < 1小时 → **跳过本次任务，不发布**，返回"间隔不足1小时，跳过"

#### 间隔检验背景

2026-09-24 发生因高频发布（7-13分钟间隔连发20条）导致平台限流、部分内容展现为0的事件。此检验步骤为强制修复措施，确保每条微头条间隔至少1小时，防止触发平台反垃圾风控。
...
11. **间隔检验**（强制·v1.1新增）：每次执行前必须检查距上一条微头条发布时间是否满1小时，不足则跳过不发布。2026-09-24因高频连发（7-13分钟间隔）导致平台限流展现归零，此规则为强制修复措施
